<div class="row border-left border-top bg-white mx-0" id="slick-{{ $wid_info['id'] }}">
    @if(!empty($data))
        @foreach($data as $content)
            <div class="border-right border-bottom col-6 col-md-{{ 12 / $wid_info['params']->columns }}">
                <div class="product-main bg-white rounded mb-0">
                    ...
                    <div class="product-info">
                        @if($wid_info['params']->showTitle)
                            ...
                            @else
                                <span class="color-red font-size-1d2 font-bold">{{ number_format($content->price) }}
                                    đ</span>
                            @endif
                        @endif
...
@if(isset($wid_info['params']->slider) && $wid_info['params']->slider == 1)
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            if (colProductFull !== 1) {
                $('#slick-{{ $wid_info["id"] }}').addClass('row');
            }
        });
    </script>
@endif
